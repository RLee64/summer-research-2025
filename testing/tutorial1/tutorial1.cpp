...
int main() {
  ...
  std::ifstream file("tutorial1.cellml");
  ...
  if (parser == nullptr) {
      std::cout << "Could not create parser";
      return 1;
  }

  libcellml::ModelPtr model = parser->parseModel(fileContent);
  ...
  if (model == nullptr) {
      std::cout << "Could not parse model";
      return 1;
  }
